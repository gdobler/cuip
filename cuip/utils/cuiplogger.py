...
# -*- coding: utf-8 -*-
"""
This modules defines cuidLogger function which
...
def cuipLogger(loggername, every="midnight", tofile=True):
    """
    ...
    formatter = logging.Formatter(
        "%(asctime)s - %(name)8s - [%(levelname)7s] - [%(processName)s] -- (%(filename)s:%(lineno)s) %(message)s", datefmt="%d-%m-%Y %H:%M:%S")
