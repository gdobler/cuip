...
for ax, mult, scale in [[tax1, 10, 100], [tax2, 5, 30]]:
    ...
    ax.bottom_axis_label("Red (%)", fontsize=fontsize, offset=0., color="r")
    data = generate_heatmap_data(scale)
    ax.heatmap(data, style="hexagonal", colormap=False)
...
tax1.scatter(mlc_per * 100, c=mlc_per, alpha=1, s=5, zorder=4)
...
tax2.scatter(points_c, c=mlc_per, s=10, alpha=1, zorder=4)
...
for ax, mult, scale in [[tax1, 10, 100], [tax2, 5, 30]]:
    ...
    ax.left_axis_label("Blue (%)", fontsize=fontsize, offset=0.17, color="b")
    ax.right_axis_label("Green (%)", fontsize=fontsize, offset=0.17, color="g")
    ax.bottom_axis_label("Red (%)", fontsize=fontsize, offset=0.0, color="r")
    data = generate_heatmap_data(scale)
    ax.heatmap(data, style="hexagonal", colormap=False)
# -- Plot the entire range.
tax1.ticks(multiple=10, offset=0.02, axes_colors=axes_colors)
...
tax1.line((25, 25, 0), (25, 65, 0), color='orange', lw=2.0)
tax1.line((25, 65, 0), (65, 25, 0), color='orange', lw=2.0)
tax1.scatter(mlc_per[idx] * 100, c=colors, cmap="jet", alpha=1, s=5,
    label="LightCurve", zorder=4)
# -- Plot the zoomed region.
tax2.set_axis_limits({'b': [25, 65], 'l': [10, 50], 'r': [25, 65]})
tax2.get_ticks_from_axis_limits(multiple=5)
tax2.set_custom_ticks(fontsize=10, offset=0.025, multiple=5,
...
tax2.scatter(points_c, c=colors, cmap="jet", s=10, alpha=1, zorder=4)
# -- Add colorbar
